...
def test_parse_args_defaults(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "sys.argv",
        ["build_training_dataset.py", "--output-dir", str(tmp_path)],
    )
...
def test_parse_args_rejects_invalid_max_samples(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "sys.argv",
        [
...
def test_process_position_clamps_cp_and_serializes_features():
    ...
    assert sample["feature_values"] == [0.1, -0.2, 0.3]
...
def test_feature_metadata_is_attached_to_parquet_schema():
    ...
    table = pa.Table.from_pylist(
        [{"fen": "x", "target_cp": 10, "source_depth": 20}]
    )
...
def test_write_table_creates_readable_parquet_with_metadata(tmp_path):
    ...
    rows = [
        ...
        },
        {
            ...
            "target_cp": -50,
            "source_depth": 25,
            "feature_values": [0.4, 0.5, 0.6],
        },
    ]

    writer = write_table(None, path, rows)
    writer.close()
    ...
    assert b"chess_ai_lab.feature_schema_hash" in table.schema.metadata


def test_manifest_records_generation_conditions_and_counts(tmp_path):
    ...
    assert manifest["actual_train_ratio"] == pytest.approx(0.897)
    assert manifest["min_depth"] == 20
    assert manifest["cp_limit"] == 1000
    assert manifest["seed"] == 42
    assert manifest["feature_count"] == len(manifest["feature_names"])
