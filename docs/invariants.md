...
AIは本書および architecture.md を前提として実装する。
...
ただし、AIは実装完了後に、実際に行った変更を明示しなければならない。
...
AIは、依頼された変更と実際に行った変更に差異がある場合、その差異を明示する。
...
AIは実装後にGit diffを確認する。

AIはdiffに含まれる変更を把握したうえで、変更内容を報告する。

意図していない変更が含まれている場合、その変更を意図した変更として扱ってはならない。

AIは無関係な変更を自動的に修正・整理・削除してはならない。

既存の未コミット変更が存在する場合、それらをユーザーの変更として扱い、勝手に変更・削除してはならない。
...
AIがGit操作を行った場合、その操作内容を明示する。

Git操作には少なくとも以下を含める。

* 実行したGit操作
* commitした場合はcommit message
* pushした場合はpush先
* branchを変更した場合は変更前後のbranch
* その他、履歴や作業状態に影響する操作

AIは、Git操作によってコード変更の内容を隠してはならない。

AIは、コード変更とGit操作を別々のものとして扱わず、1つの変更単位として報告する。

AIは以下の操作を、明示的な理由なく行ってはならない。
...
AIによる直接編集およびGit操作は、効率化のために許可する。

ただし、AIは実装・Git操作を行った事実とその内容を明示しなければならない。
...
=======
レイヤーは上
>>>>>>> 410c0be3b0d2275560dafff95e1565a610f64aff
